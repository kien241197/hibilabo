...
class HonneMiddleware(MiddlewareMixin):
    def __call__(self, request, *args, **kwargs):
        # if request.user.role is None or request.user.role.role not in [99, 20, 10]:
        #     raise PermissionDenied()
        return super().__call__(request)

class HonneTotalMiddleware(MiddlewareMixin):
    def __call__(self, request, *args, **kwargs):
        if request.user.role is None or request.user.role.role not in [30, 20]:
            raise PermissionDenied()
        return super().__call__(request)
...
class MandaraMiddleware(MiddlewareMixin):
    def __call__(self, request, *args, **kwargs):
        # if request.user.role is None or request.user.role.role not in [99, 20, 10, None]:
        #     raise PermissionDenied()
        return super().__call__(request)
